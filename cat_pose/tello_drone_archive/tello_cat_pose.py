...
KP_CONF_THRES = 0.5  # 關鍵點（kp）信心門檻（不是 bbox 偵測框門檻，bbox 見 CONF_THRES）
...
# 骨架顏色/連結定義統一從 constants.py import（見檔案開頭），跟其他共用同一套
# 分組配色的腳本（0_video_pose_viewer.py／pose_single_image_test.py／
# eda_realtime_anomaly_viewer.py／自動標註工具/auto_labeling_capture.py）保持一致
