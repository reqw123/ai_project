...
import os as _os
CONF_THRES = 0.9  # YOLO 偵測框（bbox）信心門檻（predict 的 conf）；不是關鍵點（kp）門檻，kp 見 KP_CONF_THRES
...
KP_CONF_THRES = 0.8  # 關鍵點（kp）信心門檻（不是 bbox 偵測框門檻，bbox 見 CONF_THRES）；關鍵點信心門檻：低於此值的點不列入偏移比較
...
def launch_review_gui(review_records):
    ...
    image_frame.pack(fill="both", expand=True, padx=6, pady=2)
    ...
    source_label.pack(fill="x", padx=6, pady=(3, 2))
    ...
    help_label.pack(pady=(0, 2))

    button_frame = tk.Frame(root, bg="#111827")
    button_frame.pack(fill="x", padx=6, pady=(1, 6))
    ...
    def render_current():
        ...
        if record is None:
            ...
            mark_button.configure(text="D 標記刪除", state="disabled", bg="#DC2626")
            select_button.configure(text="A 綠色選取", state="disabled", bg="#15803D")
    ...
    mark_button = make_button(
        navigation_row, "D 標記刪除", toggle_mark, "#DC2626", width=0
    )
