...
| 7 | 本文件的「[五、監控層 Node-RED 對應](#sec-5)」一節 | 看懂 4 個 json flow 各自負責什麼 |
...
| `cat_monitoring_system/server/routes.py` | 定義所有 Flask 路由（見「[六、Flask 端點總表](#sec-6)」），並在首次請求時（`_ensure_processor_started`）建立 `FrameProcessor`、註冊 `LickStagePlugin`／`ExtBodyZonePlugin` |
...
| `cat_monitoring_system/models/keypoint_kalman.py`（2026-08 新增） | 逐關鍵點等速度 Kalman filter（`KeypointKalmanFilter`/`kalman_smooth_sequence`），跟 `interpolate_missing()` 同一個插入點的替代方案；透過 `stgcn_config.yaml` 的 `SMOOTHING_KIND: "kalman"` 選用。實測結論：推論期套用、單參數/三參數匹配重訓皆未找到顯著贏過無平滑的證據，目前 `stgcn_config.yaml` 已改回 `SMOOTHING_KIND: "ema"`，這個模組保留但未啟用於正式部署（詳見 [`YOLO-Pose應用文獻與專案優化建議.md`](YOLO-Pose應用文獻與專案優化建議.md)） |
...
| `cat_monitoring_system/processors/skeleton_quality_assessment.py` | 「GCN 分類為主、幾何判斷為輔」雙重判定（見下方[3.4.1](#sec-3-4-1)），純函式模組，不依賴 `FrameProcessor`/`config.py` |
...
- `ModelPaths` / `YOLOConfig` / `STGCNConfig` → 影響「[三](#sec-3)、[3.2](#sec-3-2)/[3.3](#sec-3-3)」的感知層與分析層
