...
> **與 [`姿態估計推論前後處理技術文獻與應用建議.md`](姿態估計推論前後處理技術文獻與應用建議.md#sec-3) 的關係（2026-09-04 補充，避免誤讀為互相矛盾）**：該文件同日（2026-08-11）稍後發現了 One Euro Filter 這個新的訊號處理層候選演算法。這**不是重啟**本節「收尾」的結論——上面 A/B/C 三路線比較的是「要不要做時序遮蔽處理」的架構層級選擇（訊號處理 vs. 端到端時序模型 vs. 離線 ensemble），Kalman 的負面結果是「全域 Q/R 參數平滑對整體準確率沒有顯著幫助」；One Euro Filter 是路線 A（訊號處理層）內部、Kalman 之外的另一個候選演算法，其自適應特性理論上正好對症下藥於 Kalman 實驗診斷出的「shake 與 stop 需要的平滑強度不同」問題，尚未實測，也不能直接套用 Kalman 的負面結論。截至 2026-09-04 程式碼庫掃描未發現 `oneeuro`/`OneEuroFilter` 相關實作，這是仍未執行的獨立開放建議，非本節路線 A 的一部分。
