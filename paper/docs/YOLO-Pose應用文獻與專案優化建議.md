...
`CatIdentityConfig`/`detectors/identity_verifier.py` 目前是色彩直方圖（H-S bin）比對的做法，只能處理「這是不是目標貓」的二分類，沒有真正的多動物身分追蹤機制。DeepLabCut 的「局部 tracklet + 網路流全域縫合 + 無監督 metric learning 身分預測」是專門為「外觀相似的同物種動物互動」設計的，如果日後這個系統要支援多貓家庭（`docs/口試準備`裡提過的「個體辨識」是口試三大硬核問題之一），這篇的方法論比目前色彩直方圖的做法更適合處理「兩隻花色相近的貓在畫面裡靠近」這種情境。
