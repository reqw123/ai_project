...
`risk.level` 非 Normal 時才觸發；用「等級 + 異常類型」組成 dedup key（`flow.last_discord_alert`），同一組合不會重複發送。Webhook URL 讀自 `global.v2_user_settings.discord_webhook`（未設定時只印警告、不發送）——跟[3.1 節](#sec-3-1)上線通知用的固定 webhook URL 是**不同**的設定來源。
...
顯示合併後的行為事件時間軸，跟[3.2 節](#sec-3-2)的「行為時間紀錄」卡片（顯示未合併的原始 `behavior_log`）是兩個不同用途的顯示元件。
...
#### `健康報告` (`function`)
收到「哈基米」時：記錄請求者 ID 到 `flow.health_requester`、回覆「正在分析，請稍候」，並觸發 CSV 讀取（見[4.3](#sec-4-3)）。
...
#### `讀取CSV` (`file in`)
讀取 `C:\a\noda_data.csv`（由 `貓咪主控.json` 的 `建立CSV`/`儲存CSV` 定時寫入，見[3.4 節](#sec-3-4)）。

#### `建立GPT分析請求` (`function`)
讀取環境變數 `OPENAI_API_KEY`，組成 OpenAI Chat Completions 請求（`model: gpt-4.1-mini`），system prompt 明確定義行為類別語意、CSV 欄位說明（已對齊[3.4 節](#sec-3-4)實際 CSV 表頭：`low_conf`/`low_conf_time`，不含 `health_score`）、分析原則（禁止捏造不存在欄位/數據）與輸出格式要求。
...
立刻回應前端 `200 ok`，同時觸發[4.3 節](#sec-4-3)的 CSV 讀取／GPT 分析流程（等同 Messenger 的「哈基米」指令，但不會有 `health_requester`，所以分析完只會顯示在 Dashboard，不會推播 Messenger）。

#### `UI觸發錄影` (`http in`, `POST /ui-trigger-record`) → `UI錄影-組請求` (`function`)
立刻回應前端 `200`，同時觸發[4.4 節](#sec-4-4)的 `/video_clip` 錄影流程（`sender=null`，不推播 Messenger）。
...
手動測試用節點：注入一段固定文字，推播到程式碼裡寫死的單一 Messenger 使用者 ID，跟[3.3 節](#sec-3-3)「Discord 異常告警」是兩條互相獨立的告警管道（一個推 Discord、一個推特定人的 Messenger），非同一套機制觸發。
...
兩者最終都會匯入同一條 `讀取CSV → 建立GPT分析請求 → OpenAI GPT分析 → 解析GPT結果` 管線（見[4.3 節](#sec-4-3)），差別只在於有沒有 Messenger 使用者要推播結果。
...
專案實際上還有另外 2 條 flow（個體化基線引擎、舔舐部位分析）不在這份文件範圍內，整體 4 條 flow 的分工總覽請見 [`0_AI_專案導覽地圖.md`](0_AI_專案導覽地圖.md#sec-5) 的「五、監控層 Node-RED 對應」。
