...
| 舔毛部位模組三階段重構 | 🔄 進行中 | 依《家貓舔毛部位辨識與健康分析模組三階段工程說明書》。M1、M2 **已完成 shadow 模式**（`main` `7da2a4c`）。M5 混合尺度（`_compute_body_scale()`，移除絕對像素夾鉗）**已完成**，三支測試影片 zone coverage 皆進步（lick_31 蜷曲案例 37.8%→46.1%）；同批試的梯形方向 body-anchored 因 lick_28/lick_3 反而退步已撤回，待查根因。M3 Node-RED flow 驗證、M4 共用 PoseFrame、M5 剩餘項目（候選評分/AMBIGUOUS）、M6 bout 狀態機、M7 論文實驗未做。進度總表與下次接續點見 `plugins/lick_stage/舔拭行為二階段分析模組說明.md` 開頭「三階段重構：進度總表與下次接續」 |
| 論文撰寫 | 🔄 進行中 | |
...
- `tools/test_bone_length_stability.py` — SQA 門檻校準/診斷工具。
- `tools/eval_gcn_compare.py` — 雙模型對比評估腳本（含 ★ 贏家標示）。
