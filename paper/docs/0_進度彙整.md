...
>
> **更新日期：** 2026-07-15（部分內容已於 2026-08-11、2026-09-04 核對修正，見文中個別標註；核對範圍限於本次發現的具體錯誤，非全文重寫）
...
> - 🚨 **本文件校對狀態**：主體內容寫於 2026-07-15，2026-08-11、2026-09-04 各補了幾處核對修正（見各節內的標註），**不是全文重新校對過**。
...
1. `interpolate_missing()` — 低信心關鍵點（conf < 0.1，`threshold` 內建預設；刻意比偵測/繪圖用的 0.5 寬鬆，見 `信心值門檻總覽.md`）做時序插值補點
...
| 信心門檻命名規範（BBOX_/KPT_ 前綴） | ✅ 完成（2026-08-31） | 全專案裸 `conf`/`CONF_THRESHOLD` 統一改為 `BBOX_`（bbox 偵測）／`KPT_`（逐關鍵點）前綴，分三階段完成，過程純識別字改名、未動任何演算法邏輯或數值；完成時全專案 544 個測試通過。隨後使用者要求把所有具名 bbox/keypoint conf 門檻統一設為 0.5（`interpolate_missing()` 內部 threshold=0.1 例外，刻意保留，見下方說明）。詳見 [`信心值門檻總覽.md`](信心值門檻總覽.md) |
| 論文撰寫 | 🔄 進行中 | |
...
- 多貓追蹤（Multi-cat Tracking）：目前系統只**分析單一目標貓的行為**——`run_mode.system_mode` 有 `single`（預設，假設全程一隻貓：YOLO `max_det=1` 只偵測信心最高的一隻、不做 IoU 追蹤）/ `multi`（多貓同框時 YOLO 回傳全部 + IoU 追蹤 + 身分驗證 CNN 挑出目標貓、過濾其他貓）兩種模式，但兩種模式都只對「目標貓」做行為分類與統計；同時追蹤多隻貓各自的行為軌跡仍是未來工作
...
- [`信心值門檻總覽.md`](信心值門檻總覽.md) — bbox / keypoint / `interpolate_missing` 三種信心門檻對照，2026-08-31 命名規範重構完整說明。
- [`0_測試操作手冊.md`](0_測試操作手冊.md)、[`0_測試策略分析.md`](0_測試策略分析.md) — 測試環境（`conda activate yolo`）、指令、目前實測測試數。
