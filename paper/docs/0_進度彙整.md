...
| 舔毛部位模組三階段重構 | 🔄 進行中 | 依《家貓舔毛部位辨識與健康分析模組三階段工程說明書》。M1（來源時間契約 + 5 種 frame_state + 三分母 + Session 生命週期）、M2（事件表 / 視窗表 SQLite + 有界 latest-only 發布）**已完成 shadow 模式**（`main` `7da2a4c`）；M3 Node-RED flow 驗證、M4 共用 PoseFrame、M5 尺度正規化、M6 bout 狀態機、M7 論文實驗未做。進度總表與下次接續點見 `plugins/lick_stage/舔拭行為二階段分析模組說明.md` 開頭「三階段重構：進度總表與下次接續」 |
| 論文撰寫 | 🔄 進行中 | |
