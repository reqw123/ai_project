...
**背景**：見 [`姿態估計推論前後處理技術文獻與應用建議.md`](姿態估計推論前後處理技術文獻與應用建議.md#sec-3) 第三節、memory `project_pose_pre_post_processing`——One Euro Filter 是文獻掃描找到、目前專案沒有的技術，理論上比 Kalman 更適合處理「shake 需要低延遲、stop 需要強平滑」這種同一組全域參數顧不到兩頭的問題。
**日期**：2026-08-11
**更新：2026-09-04**（依 `stgcn_config.yaml` 現況核對補充——見下方「⚠ 現況核對」，Kalman 路線的驗證已經跑完並得到負面結論，目前部署設定實際上兩種平滑技術都沒有在用）

> [!IMPORTANT]
> **⚠ 現況核對（2026-09-04，依 `cat_monitoring_system/stgcn_config.yaml` 程式碼註解與 `stgcn_models/run_126~128` 的 `params_snapshot.json` 核對）**
>
> 本文件下方「背景」與第五節把 Kalman 描述為進行中的對照路線，但 **Kalman 這條路線（路線 A）事實上已經跑完三組實測並結案**：
> - 已訓練 3 個 Kalman 模型：`run_126`（Q=90/R=70）、`run_127`（Q=45/R=140）、`run_128`（Q=180/R=35），對應 `keypoint_kalman.py` 說明的三組候選噪聲參數。
> - 三組裡 Q90/R70（126）表現最好，但跟無平滑 baseline（`run_122`）比較，**McNemar 檢定 p=1.0，沒有統計顯著差異**。
> - 結論：Kalman 平滑沒有找到顯著贏過不平滑的證據，`stgcn_config.yaml` 的 `SMOOTHING_KIND` 已**改回 `"ema"`**（`KP_EMA_ALPHA=1.0`，等同完全不平滑），恢復現行部署行為。
> - **也就是說，目前實際部署／訓練用的模型（如 `run_122`）既沒有用 Kalman、也沒有用 One Euro Filter——兩者皆非目前系統的一部分，關鍵點平滑目前是完全停用狀態。**
>
> 這個結果直接呼應本文件第五節「如果第 1 步就沒有訊號」那段的推測——Kalman 這個「同樣插入點的替代方案」已經實測驗證過這個假設成立（瓶頸可能不在關鍵點平滑演算法選哪個），是動手實作 One Euro Filter 前應該先讀的既有證據，不是憑空的臆測。
...
3. **沒有協方差/不確定性輸出**。Kalman 的 `P` 矩陣理論上可以額外拿來當 `skeleton_quality_assessment.py` 的輔助訊號（[`YOLO-Pose應用文獻與專案優化建議.md`](YOLO-Pose應用文獻與專案優化建議.md) 提過這個構想，但從未真的實作），One Euro 沒有對應的量。目前這個構想本來就沒實作，所以不算實質損失，這裡只是誠實記一筆。
...
1. **推論期套用測試**（不需要重訓，最便宜）：`eval_accuracy_smoothing_compare.py` 已被刪除，需要先重建（見上方[第三節](#sec-3)），`SMOOTHING_CONFIGS` 加一組 One Euro 設定，先看看跟現行無平滑 baseline（122）比起來準確度如何。如果推論期套用就有明顯正面訊號，才有理由往下一步投入。
...
**如果第 1 步就沒有訊號**（One Euro 推論期套用也沒有贏過無平滑），可以直接參考 Kalman 的最終結論——見文件開頭「⚠ 現況核對」：Kalman 三組參數（126/127/128）跟無平滑 baseline（122）比較，McNemar 檢定 p=1.0，沒有顯著差異。這代表瓶頸很可能根本不在「關鍵點平滑演算法選哪個」，而是 ST-GCN 對這一層雜訊本來就不敏感，不用再重複投入到第 2、3 步。
...
- 參數起始值（[第四節](#sec-4)）純粹是文獻常見慣例，不是校準過的數字，正式使用前一定要照 Kalman 的模式重新校準。
