...
from settings_gui.widgets import _styled_button  # noqa: E402

# settings_window.py 自己定義（style.py 只留純資料常數，2026-08 按鈕配色從 4 組簡化成
# 3 組時，橘色警告色連同它的深棕字 BTN_WARN_FG 一起改名離開了「危險/BTN_DANGER」這個
# 舊稱呼——見 style.py 開頭的說明）。這個視窗是獨立於 settings_window.py 之外的
# 頂層視窗，不 import 那支幾千行的檔案，直接照同一組色碼在這裡各自定義一份。
BTN_WARN_BG = "#e67e22"
BTN_WARN_ACTIVE = "#cf711d"

_PROJECT_ROOT = _PAPER_DIR.parent                         # C:\ai_project
_CAT_IDENTITY_DIR = _PAPER_DIR / "tools" / "cat_identity"
...
DATASETS_DIR = _PAPER_DIR / "tools" / "train_data" / "cat_identity" / "datasets"
...
class IdentityTrainerWindow(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        self._del_btn = _styled_button(arow, "🗑 刪除所選模型", self._on_delete_model,
                                       BTN_WARN_BG, BTN_WARN_ACTIVE, fg=BTN_WARN_FG, font=self._font_hint,
                                       compact=True)
