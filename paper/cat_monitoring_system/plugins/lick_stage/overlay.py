...
def _draw_nose_trapezoid(
    ...
    if (
        trap_pts is None
        or np.asarray(trap_pts).ndim != 2
        or np.asarray(trap_pts).shape[0] != 4
    ):
        return
    ...
    for idx, pt in enumerate(trap_draw):
        ...
        cv2.circle(
            frame, (int(pt[0]), int(pt[1])), dot_r + 1, (0, 0, 0), -1, cv2.LINE_AA
        )
        cv2.circle(
            frame, (int(pt[0]), int(pt[1])), dot_r, (255, 255, 255), -1, cv2.LINE_AA
        )
    ...
    if hit:
        ...
        cv2.circle(
            frame, nose_pt, ripple_r, (80, 255, 255), max(2, int(3 * _ov)), cv2.LINE_AA
        )
        cv2.circle(
            frame, nose_pt, max(2, int(2 * _ov)), (255, 255, 255), -1, cv2.LINE_AA
        )
