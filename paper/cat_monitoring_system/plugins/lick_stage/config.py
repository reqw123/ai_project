...
class LickConfig:
    ...
    # ── 關鍵點信心值門檻 ──────────────────────────────────────────────────
    EAR_KPT_CONF_THRESHOLD = 0.5  # 耳朵關鍵點最低信心值，低於此值視為耳朵不可見
    NOSE_KPT_CONF_THRESHOLD = 0.5  # 鼻子關鍵點最低信心值，低於此值視為鼻子不可用（影響接觸判定與頭部朝向判斷）
    LIMB_KPT_CONF_THRESHOLD = 0.5  # 四肢關鍵點最低信心值，低於此值跳過該肢體
    BODY_KPT_CONF_THRESHOLD = 0.5  # 胸部/髖部信心值門檻，用於計算身體尺度
