...
YOLO_MODEL_PATH = str(Path(__file__).resolve().parents[2] / "yolo_models" / "v11s_128.pt")
...
def analyze_single_video(video_path: str, detector: KeypointDetector, video_index: int, classifier: BehaviorClassifier = None):
    ...
    [1]/[2] 才切換到上一支/下一支影片，或按 [ESC] 結束整個檢視——跟
    ...
    "skip"（路徑是資料夾／不存在／打不開，這支根本沒開始播放；呼叫端要自己防止「全部都無效」時無限繞圈）。
    """
...
def main():
    ...
    if run_mode == "gui":
        ...
        # 按 [1]/[2] 才切換到上一支/下一支（可循環繞回），按 [ESC] 結束整個
        # 檢視——用 while + 可前後移動的索引，不是單純從頭到尾的 for 迴圈。
        print(f"共 {n} 支影片待檢視\n")
        current_video_idx = 0
        consecutive_skips = 0  # 連續幾支「根本沒能開始播放」，全部都無效時要結束，避免無限空轉
        while True:
            video_path = video_paths[current_video_idx]
            signal = analyze_single_video(video_path, detector, current_video_idx + 1, classifier)
            print()
            consecutive_skips = consecutive_skips + 1 if signal == "skip" else 0
            if consecutive_skips >= n:
                print(f"❌ {n} 支影片全部無法播放（路徑是資料夾、不存在或打不開），結束。請檢查 VIDEO_PATHS / VIDEO_FOLDER 設定")
                break
            if signal == "quit":
                break
            elif signal == "prev":
