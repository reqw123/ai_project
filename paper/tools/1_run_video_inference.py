...
DISPLAY_FULLSCREEN = (DISPLAY_RESOLUTION == "1080p")  # 1080p ＝ 全螢幕（鋪滿 1920x1080 螢幕，ESC 結束）；720p 維持一般視窗。想要「視窗式的 1080p」就把這行改成 False
WINDOW_SCALE_STEP = 0.10  # Ctrl+加號／減號每按一次縮放的幅度（只在一般視窗有效，全螢幕不縮放）
...
DRAW_KP_CONF_THRESHOLD = 0.7    # 畫骨架線段與關鍵點圓點用門檻（>此值才畫）
import os as _os
...
def resize_with_letterbox(image, target_size):
    ...
    # 縮小用 INTER_AREA（不會有鋸齒／摩爾紋）、放大用 INTER_CUBIC（比 LINEAR 銳利），
    # 兩種方向都盡量不損失畫質；只作用在預覽畫面，不影響送進模型的原始畫面。
    if (new_w, new_h) == (src_w, src_h):
        resized = image  # 來源尺寸就是目標尺寸（例如 1080p 影片 + 1080p 預覽）：不需要縮放，省一次全幅運算
    else:
    ...
    while not stop_requested:
        ...
        if is_stream_url:
            ...
            if cap is None or not cap.isOpened():
                ...
                open_fail_streak += 1
                if open_fail_streak >= len(video_paths):
                    print("❌ 播放清單裡每一項都無法開啟，結束")
                    break
                if is_stats_mode:
                    current_video_idx += 1
                    if current_video_idx >= len(video_paths):
        ...
        print("-" * 60)
        # 切換影片時，最後一行固定印出「目前影片的檔名」（串流網址則印完整網址）。
        # flush=True：stdout 被導向 pipe（例如設定視窗的模擬終端機）時 Python 會做區塊緩衝，
        # 不立刻 flush 的話這行要等緩衝區滿了才出現，看起來就不是「切換當下」同步顯示。
        print(f"▶ 目前影片：{video_path if is_stream_url else Path(video_path).name}", flush=True)
        ...
        while True:
            ...
            if display_window:
                ...
                _nav = (f"[{current_folder_key.upper()}]{_fn}  "
                        ...
                        f"| z WALK  x LICK  c SCRATCH  v SHAKE  b STOP  | Shift+A WALK B LICK C SCRATCH D SHAKE E STOP 移入  | a/d 逐幀  | t 跳轉")
                _fs = 0.42 * _ui
                _th = max(1, int(_ui))
                ...
                if _zoom:
                    window_scale = _window_zoom.clamp_scale(
                        window_scale, _zoom, WINDOW_SCALE_STEP, WINDOW_SCALE_MIN, WINDOW_SCALE_MAX,
                    )
                    cv2.resizeWindow(
