...
控制鍵（SHOW_PREVIEW=True 時）：ESC=退出　space=暫停/繼續
...
def detect_all_instances(model, frame):
    ...
    results = model.predict(frame, imgsz=YOLO_IMGSZ, conf=YOLO_CONF_THRESHOLD, quantize=16 if str(INFERENCE_DEVICE).lower().startswith("cuda") else None, verbose=False)[0]
    out = []
...
def _render_preview(frame, decisions, profiles, frame_idx, video_label, paused):
    ...
    return resize_with_letterbox(disp, DISPLAY_SIZE)
...
def cmd_verify(sources, show):
    ...
    if show:
        ...
        cv2.resizeWindow(PREVIEW_WINDOW_NAME, *DISPLAY_SIZE)
        if DISPLAY_FULLSCREEN:
            cv2.setWindowProperty(PREVIEW_WINDOW_NAME, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
        print("控制鍵: ESC=退出　space=暫停/繼續　a/d=逐幀後退/前進(暫停時)　1=上一部影片　2=下一部影片")
