...
# 輸出根資料夾：CSV 與下載的影片都放在這底下，只需要改這一行
OUTPUT_ROOT = r"C:\ai_project\paper\output\fetch_cat_behavior_videos"
OUTPUT_CSV = os.path.join(OUTPUT_ROOT, "cat_behavior_videos.csv")

DOWNLOAD_VIDEOS = True      # 是否自動下載影片檔案
DOWNLOAD_BASE_DIR = os.path.join(OUTPUT_ROOT, "downloaded_videos")   # 影片存放的根資料夾（原為相對路徑，改成絕對路徑避免依執行時的工作目錄而跑到不同地方，也避免大量影片混進原始碼資料夾）
