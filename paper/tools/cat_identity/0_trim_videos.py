...
執行環境：需要 ultralytics + ffmpeg。本機用專案 conda 環境：
    & "C:\\Users\\homec\\anaconda3\\envs\\yolo_new\\python.exe" 0_trim_videos.py
"""

# ═══════════════════════════════════════════════════════════════
#  設定區
# ═══════════════════════════════════════════════════════════════
from pathlib import Path  # 下面的模型路徑以本檔位置推算專案根目錄，設定區在 import 區塊之前，所以在這裡先匯入
YOLO_MODEL_PATH = str(Path(__file__).resolve().parents[3] / "yolo_models" / "v11s_149.pt")
...
OUTPUT_ROOT = _pathlib.Path(r"C:\ai_project\paper\tools\train_data\cat_identity\trimmed")
...
if _MISSING:
    ...
    print(r'   本機請用：& "C:\Users\homec\anaconda3\envs\yolo_new\python.exe" 0_trim_videos.py')
    sys.exit(1)
...
def find_ffmpeg():
    for name in ("ffmpeg", "ffmpeg.exe"):
        ...
        if p:
            return p
    for cand in (
        r"C:\Users\homec\anaconda3\Library\bin\ffmpeg.exe",
        r"C:\Users\homec\anaconda3\envs\yolo_new\Library\bin\ffmpeg.exe",
    ):
        if Path(cand).exists():
            return cand
    return None
