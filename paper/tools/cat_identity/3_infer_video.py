...
執行環境：需要 torch/torchvision/ultralytics（CUDA 版）。本機用專案 conda 環境：
    & "C:\\Users\\homec\\anaconda3\\envs\\yolo_new\\python.exe" 3_infer_video.py
...
YOLO_MODEL_PATH = str(Path(__file__).resolve().parents[3] / "yolo_models" / "v11s_149.pt")        # 抓貓 bbox 用（偵測/pose 皆可）
...
if _MISSING:
    ...
    print(r'   本機請用：& "C:\Users\homec\anaconda3\envs\yolo_new\python.exe" 3_infer_video.py')
    sys.exit(1)
