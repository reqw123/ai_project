...
DRAW_KP_CONF_THRESHOLD = 0.5  # 關鍵點（kp）信心門檻：低於此值的關鍵點不畫（不是 bbox 偵測框門檻）
import os as _os
...
# 這支腳本是左右兩個畫面並排，解析度指「兩個畫面合計」的大小（每個畫面寬度減半）。
...
_pane_w, _pane_h = _DISPLAY_RESOLUTION_PRESETS[DISPLAY_RESOLUTION]
DISPLAY_SIZE = (_pane_w // 2, _pane_h)  # 每個畫面（寬, 高）
...
def _tile_borderless_windows(titles, size):
    """1080p：把並排的視窗拿掉標題列與邊框，由左到右緊貼、鋪滿整個螢幕（Windows 限定；失敗就維持一般視窗）。"""
    try:
        import ctypes
        ...
        for i, title in enumerate(titles):
            ...
            user32.SetWindowLongW(hwnd, -16, style & ~(0x00C00000 | 0x00040000))  # 去掉 WS_CAPTION、WS_THICKFRAME
            user32.SetWindowPos(hwnd, 0, i * w, 0, w, h, 0x0020 | 0x0040)  # SWP_FRAMECHANGED | SWP_SHOWWINDOW
    except Exception as e:  # noqa: BLE001
        print(f"⚠ 無法排成無邊框並排（{e}），維持一般視窗")
...
def main():
    ...
    print("控制: ESC=退出  space=暫停  r=重置目前影片  1/2=上/下一部影片  a/d=後退/前進一幀（雙視窗同步）" +
    ...
    if DISPLAY_FULLSCREEN:
        _tile_borderless_windows((win_a, win_b), DISPLAY_SIZE)
