...
def main():
    ...
    print("控制: ESC=離開, space=暫停/播放, a/d=暫停時逐幀後退/前進, i=資訊顯示, m=切換顯示模式(僅骨架/完整), Ctrl+加號=放大, Ctrl+減號=縮小, 2=下一部, 1=上一部, r=重置本片")
    ...
    def _apply_window_scale():
        if DISPLAY_FULLSCREEN:  # 全螢幕時 +/- 縮放無效（resizeWindow 會讓視窗跳出全螢幕）
            return
    ...
    def _handle_key(key, cap_obj, in_pause_loop=False):
        ...
        # Ctrl+加號／減號縮放視窗：waitKey 收不到 Ctrl 組合鍵，改成每幀輪詢鍵盤狀態（見 _window_zoom.py）
        _zoom = _window_zoom.poll(WINDOW_NAME)
        if _zoom:
        ...
        if key == 27 or key == 3:  # 3 = Ctrl+C 被 OpenCV 視窗攔截
            print("\n使用者中斷：ESC / Ctrl+C")
            stop_all = True
            is_paused = False
            return "break"
