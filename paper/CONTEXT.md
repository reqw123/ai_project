...
- 尚無真實健康異常事件的 ground truth 可驗證風險分數的敏感度/特異度——目前也**沒有可信的真實連續即時監測多日資料**（`.node-red/context/global.json` 裡的歷史記錄來自離線影片批次測試，不適用於基線驗證），這比「資料量不夠」更根本，直接影響自我一致性切半驗證、事件標記弱 GT 等驗證方法能否執行
